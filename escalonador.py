...
class Escalonador:
    ...
    def separate(self, infos: list):
        ...
        for i in range(len(infos)):
            ...
            page_sequence_as_int = list(map(int, data[5].split(" ")))
            
            newprocess = Process(*splitting, maxPages=self.maxProcessPages, page_sequence=page_sequence_as_int)
            self.processes.append(newprocess)
            self.tempoex.append(newprocess.exectime)
    ...
    def CFS(self):
        ...
        while not tree.is_empty() or waiting_queue:
            # Fase 1: Adicionar processos que ficaram prontos à árvore de execução.
            for process in waiting_queue[:]:
                if process.beggining <= self.clock:
            ...
            # Fase 2: Selecionar e REMOVER o processo mais justo (menor vruntime).
            min_key = tree.min_key()
            execprocess = tree.pop(min_key)
            
            # Fase 3: Executar o processo pelo seu quantum ou até terminar.
            is_done = False
            for _ in range(self.frac):
                ...
                if execprocess.page_sequence:
                    self.memory_manager.accessPage(execprocess)
                
                execprocess.alreadyexec += 1
                execprocess.vruntime += execprocess.priority
                self.clock += 1

            # Fase 4: Chamar o envelhecimento do NFU após o quantum do processo.
            if self.memory_manager.algSubstituicao == "NFU":
                self.memory_manager.envelhecer_contadores()
            
            # Fase 5: Decidir o destino do processo.
            if not is_done:
                # Se não terminou, REINSERE na árvore com vruntime atualizado.
                tree.insert((execprocess.vruntime, execprocess.pid), execprocess)
            else:
                # Se terminou, libera seus recursos na memória.
